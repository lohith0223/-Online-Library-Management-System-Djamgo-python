...
def ADD_BOOKS(request):
    ...
    if request.method == "POST":
        ...
        # Validate catid and authid are not empty and are integers
        if not catid or not authid:
            messages.error(request, 'Category and Author are required')
        ...
        try:
            ...
            messages.error(request, 'Invalid category or author')
            return redirect('add_books')
...
def ISSUE_BOOK(request):
    if request.method == 'POST':
        ...
        try:
            ...
            book.isIssued = '1'  # Update to match your DB field type
            book.save()
        ...
        students = Student.objects.all()
        books = Book.objects.filter(Q(isIssued='0') | Q(isIssued='Return'))  # Adjust if using boolean
